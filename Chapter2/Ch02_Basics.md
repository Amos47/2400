...
And this is where Task comes in. Tools and Tasks go hand in hand. A context might well be assembled out of the need to complete a task with a set of tools with one or more actors on the Team. In our discussion of the classrooom context, the Task is to learn something. The Team is assembled in a Terroir with various technology Tools in order to complete a task: learning.

Finally, we consider Trail or history. I am going to argue here that a context is dependent on one or more previous contexts in order to exist. The classroom context, for example, has a very specific history. Beyond the obvious precursors of an existing university with students and a physical classroom with proper furniture, technology Tools might need to be installed, lecturers must have knowledge and experience to impart, and students need to have the prerequisite tools and skills to allow them to learn. I am hard pressed to think of a context that is not dependent on a previous context - that is since the Big Bang... 
 
What is interesting about the 6 Ts is that it is not necessary to consider each or all of them in determining a context. Sometimes only the top three are necessary to establish context. Sometimes all are involved. But consider that there must be three at least to frame a context? 
